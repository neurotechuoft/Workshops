"""
The main game and rendering logic.
...
from game.event_layer_provider import __post_input_events, INPUT_EVENT
...
def show_welcome_animation():
    ...
    while True:
        __post_input_events()  # Grab the event from the input provider.
...
def main_game(movementInfo):
    ...
    while True:
        ...
        crashTest = check_crash(
            {'x': playerx, 'y': playery, 'index': playerIndex}, upperPipes,
            lowerPipes)
        if crashTest[0]:
            return {
                'y': playery,
...
def check_crash(player, upperPipes, lowerPipes):
    """returns True if player colldes with base or pipes."""
    pi = player['index']
    ...
    if player['y'] + player['h'] >= BASEY - 1:
        ...
        for uPipe, lPipe in zip(upperPipes, lowerPipes):
            ...
            uCollide = pixel_collision(playerRect, uPipeRect, pHitMask,
                                       uHitmask)
            lCollide = pixel_collision(playerRect, lPipeRect, pHitMask,
                                       lHitmask)
