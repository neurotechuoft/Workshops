...
double start_time;
double elapsed_time;
int num_iters = 10000; // Take 10000 samples to calculate the effective sampling rate of the system
int iter = 0;
boolean loop_forever = true, done = false;
...
void loop() {
  ...
  decision = (sample > threshold); // If sample > threshold, this statement is 1, otherwise, it is 0;
  ...
  if (Serial.available() > 0) {
    ...
    ; // Do nothing if the game is not ready to receive
  }
...
void calculate_sampling_rate() {
  double elapsed_time = micros() - start_time;

  // Convert from microseconds to seconds
  double elapsed_time_in_s = elapsed_time / MICROSEC_TO_SEC;

  // Average reading time in microseconds
  double avg_read_time = elapsed_time / num_iters;

  // Convert from microseconds to seconds
  double avg_read_time_in_s = avg_read_time / MICROSEC_TO_SEC;

  // Sampling rate = 1 sec / (time per reading)
  double samples_per_sec = 1 / avg_read_time_in_s;

  Serial.print("Elapsed_time: ");
  Serial.print(elapsed_time_in_s, PRINT_PERCISION);
  Serial.println(" s");

  Serial.print("Average reading time: ");
  Serial.print(avg_read_time_in_s, PRINT_PERCISION);
  Serial.println(" s");

  Serial.print("Sample rate: ");
  Serial.print(samples_per_sec);
  Serial.println(" Hz");
